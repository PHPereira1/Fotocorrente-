...
num_QB_left  = 10		!Number of well/barrier of the Bragg mirror
L_qw_left    = 10.8d0 	!Width of lateral quantum well [nm]
L_qb_left    = 10.2d0 	!Width of quantum barrier [nm]

! central quantum well
L_cqw        = 12.8d0 	!Width of central quantum well [nm]

! quantum bragg mirror to right of central quantum well
num_QB_right = 10 		!Number of well/barrier of the Bragg mirror
L_qw_right   = 10.8d0 	!Width of lateral quantum well [nm]
L_qb_right   = 10.2d0 	!Width of quantum barrier [nm]
...
fs= 0*efs_InGaAs
call potencial(fs, L_left, num_qb_left, L_qw_left, L_qb_left, L_cqw, num_qb_right, L_qw_right, L_qb_right, x_cut, n)
...
!----------------------------------------------------------------
	...
	! quando ha explosao da funcao de onda no extremo 
	!----------------------------------------------------------------	
	...
	do i=2, x_cut + 1
		...
		am = (m(i-1) + m(i))/2.d0 ! media das massas do eletron no ponto i-1
		aux1 = (2*am*(Ener-v(i-1)))/hbar/hbar
		ki(i-1) = cdsqrt(aux1)
		
		am = (m(i) + m(i+1))/2.d0 ! media das massas do eletron no ponto i
		aux1 = (2*am*(Ener-v(i)))/hbar/hbar
		ki(i) = cdsqrt(aux1)
	
		am = (m(i+1) + m(i+2))/2.d0 ! media das massas do eletron no ponto i+1
		aux1 = (2*am*(Ener-v(i+1)))/hbar/hbar
		ki(i+1) = cdsqrt(aux1)
	...
	do i=n-1,x_cut + 1, -1 
		...
		am = (m(i-2) + m(i-1))/2.d0 ! media das massas do eletron no ponto i-1
		aux1 = (2*am*(Ener-v(i-1)))/hbar/hbar
		ki(i-1) = cdsqrt(aux1)
		...
		am = (m(i) + m(i+1))/2.d0 ! media das massas do eletron no ponto i+1
		aux1 = (2*am*(Ener-v(i+1)))/hbar/hbar
		ki(i+1) = cdsqrt(aux1)
...
!                     potencial 
!---------------------------------------------------------------
	subroutine potencial(fs, L_left, num_qb_left, L_qw_left, L_qb_left, L_cqw, num_qb_right, L_qw_right, L_qb_right, x_cut, n)
		use var
	    Implicit none
...
10      continue
		...
		do i = j, n	
			if(x(i).ge.-L.and.x(i).le.-L0)then
				v(i) = Vo
			else 
				go to 20
			end if
...
!----------------------------------------------------------------------
		...
		do j = i, n
			L0 = L0_left - n_qb_left*(L_qb_left+L_qw_left)
			if(x(j).ge.-L0.and.x(j).le.-L0+L_qw_left)then
				v(j) = 0
			else if(x(j).ge.-L0+L_qw_left.and.x(j).le.-L0+L_qw_left+L_qb_left)then
				v(j) = vo
			else
				v(j) = 0
				n_qb_left = n_qb_left + 1 ! adicione uma repeticao poco/barreira no potencial
			...
			if(k.gt.num_qb_left) go to 30 ! se o numero de repeticao (k) for igual a numero fornecido no input (um_qb_left), finalize o looping
		end do
30      continue
...
!----------------------------------------------------------------------		
		...
		do i = j, n
			if(x(i).ge.0.d0.and.x(i).le.L_cqw)then
				v(i) = 0
			else
			go to 40
		end if
...
!----------------------------------------------------------------------		
		...
		if(x(j).ge.L0.and.x(j).le.L0+L_qb_right)then
			v(j) = vo
			!write(*,*)"passei!"
		else if(x(j).ge.L0+L_qb_right.and.x(j).le.L0+L_qb_right+L_qw_right)then
			v(j) = 0.d0
		else
			v(j) = vo!0.d0
			n_qb_right = n_qb_right + 1
		...
		if(k.gt.num_qb_right) go to 50	
		end do
50		continue

!----------------------------------------------------------------------
		do i = j, n	
			if(x(i).ge.L0.and.x(i).le.L+L_cqw)then
				v(i) = Vo
			else 
				go to 60
			end if
